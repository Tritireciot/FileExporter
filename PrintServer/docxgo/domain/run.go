...
type Run interface {
	...
	// Superscript / Subscript пока так просто может кончено придется что-то круче добавить
	Superscript() bool
	SetSuperscript(superscript bool) error
	Subscript() bool
	SetSubscript(subscript bool) error
}
